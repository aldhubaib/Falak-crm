    ...
    <div className="animate-in slide-in-from-bottom-2 fade-in-0 duration-200 w-full max-w-[420px] rounded-xl border border-destructive/30 bg-background shadow-2xl overflow-hidden">
      ...
      {(error.stack || error.context) && (
        <div className="px-4">
          <button
            ...
            className="flex items-center gap-1 text-sub text-muted-foreground hover:text-foreground transition-colors min-h-touch"
          >
          ...
          {expanded && (
            <div className="mt-2 rounded-lg bg-muted/30 border border-border p-3 max-h-[200px] overflow-auto">
              {error.code && (
                <p className="text-sub font-mono text-muted-foreground mb-1">
              ...
              {error.context && Object.keys(error.context).length > 0 && (
                <div className="mb-2">
                  {Object.entries(error.context).map(([key, val]) => (
                    <p key={key} className="text-sub font-mono text-muted-foreground">
      ...
      <div className="flex items-center justify-between px-4 py-3 mt-1">
        ...
        <button
          onClick={handleCopy}
          className={cn(
            "min-h-touch px-3 rounded-full border text-sub font-medium flex items-center gap-1.5 transition-all",
            copied
...
export function ErrorToast() {
  ...
  return (
    <div className="fixed bottom-4 right-4 z-[9999] flex flex-col-reverse gap-2 items-end">
      ...
      {errors.length > 3 && (
        <p className="text-sub text-muted-foreground px-3 py-1 rounded-full bg-background border border-border">
