    ...
    <div className="p-6">
      ...
      <div className="flex items-center gap-3 h-12 mb-6">
        ...
        <div className="flex-1">
          <h1 className="text-heading font-semibold text-foreground">{invoice.number}</h1>
      ...
      <div className="grid grid-cols-1 @lg:grid-cols-3 gap-6">
        ...
        <div className="@lg:col-span-2 rounded-xl border border-border bg-card p-6">
          {/* Line Items */}
          <div className="mb-6">
            <div className="grid grid-cols-[1fr_60px_100px_100px] gap-2 text-sub text-muted-foreground font-medium mb-2 px-2">
            ...
            {invoice.items.map((item) => (
              <div
                key={item.id}
                className="grid grid-cols-[1fr_60px_100px_100px] gap-2 text-sub py-2.5 px-2 rounded-lg hover:bg-muted/50"
              >
          ...
          <div className="border-t border-border pt-4 space-y-2">
            ...
            <div className="flex justify-between text-body font-semibold pt-2 border-t border-border">
        ...
        <div className="space-y-4">
          <div className="rounded-xl border border-border bg-card p-4">
            <h3 className="text-body font-medium text-foreground mb-3">Status</h3>
            <StatusTimeline invoice={invoice} />
          </div>

          <div className="rounded-xl border border-border bg-card p-4">
            <h3 className="text-body font-medium text-foreground mb-3">Public Link</h3>
            ...
            <div className="flex items-center gap-2">
              <code className="flex-1 text-sub bg-muted px-2 py-1.5 rounded truncate text-foreground">
                /inv/{invoice.publicToken}
              </code>
              <a
                href={`/inv/${invoice.publicToken}`}
                target="_blank"
                className="w-icon-btn h-icon-btn rounded-lg flex items-center justify-center text-muted-foreground hover:text-primary hover:bg-primary/10 transition-colors"
              >
          ...
          {invoice.rejectionNote && (
            <div className="rounded-xl border border-destructive/30 bg-destructive/5 p-4">
              <h3 className="text-body font-medium text-destructive mb-1">Rejection Reason</h3>
              <p className="text-sub text-foreground">{invoice.rejectionNote}</p>
            </div>
          )}
        </div>
      </div>
    ...
    <div className="space-y-2">
      {steps.map((step, i) => (
        <div key={i} className="flex items-center gap-2">
          ...
          />
          <span className={`text-sub ${step.done ? "text-foreground" : "text-muted-foreground"}`}>
            {step.label}
          </span>
          {step.date && (
