...
function SlotRow({
  ...
  const onFile = async (files: FileList | null) => {
    ...
    try {
      ...
      fd.set("slot", slot.id);
      fd.set("file", file);
      await run("setBrandingAsset", () => setBrandingAsset(fd));
    } catch (e) {
  ...
  const onRemove = async () => {
    ...
    setBusy(true);
    try {
      await run("removeBrandingAsset", () => removeBrandingAsset(slot.id));
    } finally {
      setBusy(false);
    }
  };
  ...
  return (
    <section className="rounded-card border border-border/60 bg-surface p-4">
