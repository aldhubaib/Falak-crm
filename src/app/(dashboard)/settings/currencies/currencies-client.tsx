  ...
  return (
    <div className="p-6">
      <div className="flex items-center gap-3 h-12 mb-6">
        ...
        <h1 className="text-heading font-semibold text-foreground flex-1">Currencies</h1>
      ...
      <div className="rounded-lg border border-border bg-muted/20 p-4 mb-6">
        <div className="flex items-center gap-2 mb-1">
          ...
          <p className="text-sub font-medium text-foreground">Base Currency: {baseCurrency}</p>
        </div>
        ...
        {currencies.map((currency) => (
          <div key={currency.id} className="rounded-xl border border-border bg-card p-4">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-full bg-muted flex items-center justify-center text-body font-semibold text-foreground">
                {currency.symbol}
              </div>
              <div className="flex-1">
                <div className="flex items-center gap-2">
                  <p className="text-body font-medium text-foreground">{currency.code}</p>
                  {currency.isBase && (
                    <span className="text-label px-1.5 py-0.5 rounded bg-amber-500/10 text-amber-500 font-medium">
                      BASE
                    </span>
                  )}
                </div>
                <p className="text-sub text-muted-foreground">{currency.name}</p>
              </div>
              {!currency.isBase && (
                <div className="flex items-center gap-1">
                  <button
                    onClick={() => handleSetBase(currency.code)}
                    className="w-icon-btn h-icon-btn rounded-lg flex items-center justify-center text-muted-foreground hover:text-amber-500 hover:bg-amber-500/10 transition-colors"
                    title="Set as base currency"
                  >
                    <Star className="w-icon-sm h-icon-sm" />
                  </button>
                  <button
              ...
              <div className="mt-3 pt-3 border-t border-border">
                ...
                {latestRates[currency.code] && (
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <p className="text-sub text-foreground font-medium">
                  ...
                  return (
                    <div className="mb-3">
                      <button
                        ...
                        className="flex items-center gap-1.5 text-sub text-muted-foreground hover:text-foreground transition-colors"
                      >
                      ...
                      {showHistory === currency.code && (
                        <div className="mt-2 rounded-lg border border-border bg-muted/20 overflow-hidden">
                          <div className="grid grid-cols-3 gap-2 px-3 py-1.5 border-b border-border">
                            <span className="text-label font-medium text-muted-foreground uppercase">Rate</span>
                            <span className="text-label font-medium text-muted-foreground uppercase">Effective Date</span>
                            <span className="text-label font-medium text-muted-foreground uppercase">Status</span>
                          </div>
                          {history.map((entry, idx) => (
                            <div key={entry.id} className="grid grid-cols-3 gap-2 px-3 py-2 border-b border-border last:border-0">
                              ...
                              <span className={`text-label font-medium ${idx === 0 ? "text-green-400" : "text-muted-foreground"}`}>
                ...
                {editingRate !== currency.code ? (
                    ...
                    className="min-h-touch px-3 rounded-full border border-border text-button font-medium text-muted-foreground hover:text-foreground hover:bg-muted/30 transition-colors flex items-center gap-1.5"
                  >
                  ...
                  <div className="rounded-lg border border-border bg-muted/20 p-3 space-y-2">
                    <p className="text-sub font-medium text-muted-foreground uppercase tracking-wider">New Rate</p>
                    <div className="flex items-center gap-2">
                      <span className="text-sub text-muted-foreground whitespace-nowrap">1 {currency.code} =</span>
                      <input
                        type="number"
                        step="0.000001"
                        min="0"
                        ...
                        className="flex-1 h-input px-3 rounded-lg border border-border bg-black text-body text-foreground placeholder:text-muted-foreground/50 focus:outline-none focus:border-ring transition-colors"
                      />
                      <span className="text-sub text-muted-foreground">{baseCurrency}</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <Calendar className="w-3 h-3 text-muted-foreground" />
                      <span className="text-sub text-muted-foreground whitespace-nowrap">Effective as of</span>
                      <input
                        type="date"
                        ...
                        className="flex-1 h-input px-3 rounded-lg border border-border bg-black text-body text-foreground focus:outline-none focus:border-ring transition-colors"
                      />
                    </div>
                    <div className="flex items-center gap-2 justify-end pt-1">
                      <button
                        ...
                        className="min-h-touch px-3 rounded-full border border-border text-button font-medium text-muted-foreground hover:text-foreground hover:bg-muted/30 transition-colors"
                      >
                        Cancel
                      </button>
                      <button
                        ...
                        className="min-h-touch px-3 rounded-full border border-border text-button font-medium text-foreground hover:bg-muted/30 transition-colors disabled:opacity-50 flex items-center gap-1"
                      >
      ...
      {currencies.length === 0 && (
        <div className="rounded-xl border border-border bg-card p-8 text-center">
          ...
          <p className="text-body text-muted-foreground mb-2">No currencies configured.</p>
          <p className="text-sub text-muted-foreground">Add your base currency (KWD) first, then add others.</p>
        </div>
      )}
      ...
      {showAdd && (
        <div className="fixed inset-0 z-[100] flex items-end lg:items-center justify-center">
          ...
          <div className="relative w-full lg:w-[480px] max-w-md max-h-[85vh] bg-background border border-border rounded-t-2xl lg:rounded-2xl overflow-hidden flex flex-col animate-in slide-in-from-bottom lg:zoom-in-95 duration-200 shadow-2xl p-5">
              <h3 className="text-subheading font-semibold text-foreground mb-4">Add Currency</h3>
              <div className="space-y-1 max-h-[300px] overflow-auto">
                {unusedCurrencies.map((c) => (
                  <button
                    key={c.code}
                    onClick={() => handleAdd(c.code, c.name, c.symbol)}
                    className="w-full flex items-center gap-3 p-3 rounded-lg hover:bg-muted/50 transition-colors text-left min-h-touch"
                  >
                    <span className="w-8 h-8 rounded-full bg-muted flex items-center justify-center text-sub font-semibold">
                      {c.symbol}
                    </span>
                    <div>
                      <p className="text-body text-foreground font-medium">{c.code}</p>
                      <p className="text-sub text-muted-foreground">{c.name}</p>
                    </div>
                  </button>
                ))}
              </div>
