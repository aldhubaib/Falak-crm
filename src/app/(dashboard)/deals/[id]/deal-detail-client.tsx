    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-3 px-6 h-12 border-b border-border/50 shrink-0">
        ...
        <div className="flex-1 min-w-0">
          <h1 className="text-[14px] font-semibold text-foreground truncate leading-tight">{deal.title}</h1>
        ...
        {deal.project ? (
          <Link
            ...
            className="h-7 px-2.5 rounded-lg bg-purple/15 text-[11px] font-medium text-purple hover:bg-purple/25 transition-colors flex items-center gap-1.5 no-underline shrink-0"
          >
            View Project
          </Link>
  ...
  return (
    <Section title="Price Table" count={deal.items.length}>
      <div className="border border-border rounded-lg">
        <div className="grid grid-cols-[24px_1fr_100px_100px_100px_32px] gap-0 bg-muted/50 text-[10px] font-medium text-muted-foreground uppercase tracking-wider">
          <div />
  ...
  return (
    <div className="grid grid-cols-[24px_1fr_100px_100px_100px_32px] gap-0 border-t border-border items-center group">
      ...
      <div className="px-3 py-2.5">
        <p className="text-[13px] text-foreground font-medium">{item.service.name}</p>
        {item.description && (
    ...
    <div className="grid grid-cols-[24px_1fr_100px_100px_100px_32px] gap-0 border-t border-border items-center">
      ...
      <div className="px-3 py-2.5 relative">
        <input
          ...
          disabled={saving}
          className="w-full text-[13px] bg-transparent text-foreground placeholder:text-muted-foreground/40 focus:outline-none"
        />
        {showDropdown && (
          <>
            ...
            <div className="absolute z-50 left-0 right-0 top-full mt-1 rounded-lg border border-border bg-black shadow-lg overflow-hidden">
              <div className="max-h-[240px] overflow-y-auto">
                {filtered.map((s) => (
                  ...
                  >
                    <p className="text-[13px] text-foreground font-medium">{s.name}</p>
                ...
                {filtered.length === 0 && (
                  <p className="px-3 py-3 text-[12px] text-muted-foreground">No items found.</p>
                )}
              </div>
            </div>
          </>
      ...
      <div className="px-3 py-2.5 text-right text-[13px] text-muted-foreground/40 tabular-nums">{qty}</div>
      <div className="px-3 py-2.5 text-right text-[13px] text-muted-foreground/40 tabular-nums">{parseFloat(rate).toFixed(2)}</div>
      <div className="px-3 py-2.5 text-right text-[13px] text-muted-foreground/40 tabular-nums">{amount.toFixed(3)}</div>
      <div />
    </div>
  );
}
  ...
  return (
    <div className="mt-4 flex justify-end">
      <div className="w-[340px] space-y-2.5">
            ...
            {canEdit && (
              <div className="flex items-center rounded-md border border-border">
                <input
                  ...
                  className="w-14 h-7 px-2 text-[12px] text-foreground bg-background text-right tabular-nums focus:outline-none"
                />
                <div className="relative">
                  <button
                    type="button"
                    onClick={() => setDropdownOpen(!dropdownOpen)}
                    className="h-7 px-2 text-[11px] text-muted-foreground bg-muted/50 border-l border-border hover:bg-muted transition-colors flex items-center gap-0.5"
                  >
                    ...
                    <>
                      ...
                      <div className="absolute z-50 right-0 top-full mt-1 rounded-md border border-border bg-black shadow-lg overflow-hidden">
                        <button
                          ...
                          className={`w-full px-3 py-1.5 text-[12px] text-left hover:bg-muted/30 transition-colors ${discType === "percent" ? "text-primary bg-primary/10" : "text-foreground"}`}
                        >
                          %
                        </button>
                        <button
                          ...
                          className={`w-full px-3 py-1.5 text-[12px] text-left hover:bg-muted/30 transition-colors ${discType === "fixed" ? "text-primary bg-primary/10" : "text-foreground"}`}
                        >
                          {currency}
                        </button>
                      </div>
        ...
        <div className="flex justify-between items-center text-[13px] font-semibold pt-2 border-t border-border">
