...
// invoices). Matches the Lovable design: Edit / WhatsApp / Share are always
// present, items that don't fit the available width collapse into a "..."
// menu, and actions without a real handler yet show a "not available" notice
// on click instead of silently doing nothing.

import { useEffect, useState, type ReactNode } from "react";
import { Edit3, Info, MessageCircle, Share2 } from "lucide-react";
import { OverflowToolbar, type OverflowItem } from "@/components/overflow-toolbar";
...
export function EntityPreviewToolbar({
  ...
  const items: OverflowItem[] = [
    ...
    ...extra,
  ].map((it) => ({ ...it, onClick: it.onClick ?? (() => setNotice(true)) }));
