...
// Centrifugo is the real-time transport. This module is server-only: it mints the
// short-lived JWTs browsers use to connect/subscribe and publishes events to
// Centrifugo's HTTP API. It is best-effort — if env vars are missing, publishes
// are no-ops so the app keeps working on its polling/refresh fallbacks.
